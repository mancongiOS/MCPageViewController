...
open class MCPageViewController: UIViewController {
     ...
     * 自定义item样式
     */
    public func initCustomPageWithConfig(_ config: MCPageConfig,items: [UIButton]) {
    ...
    private lazy var indicatorView: UIView = {
        let view = UIView()
        view.backgroundColor = config.selectedColor
        return view
    }()
...
private extension MCPageViewController {
    ...
    private func titleButtonClicked(btn:UIButton) {
        ...
        let width = title.MCPageString_getWidth(font: config.blockFont, height: config.blockFont)
        ...
        for button in titleButtonArray {
            ...
            if tagNum != button.tag {
                ...
                UIView.animate(withDuration: 0.15, delay: 0.0, options: UIViewAnimationOptions.layoutSubviews, animations: {
                    ...
                    self.indicatorView.bounds = CGRect.init(x: 0, y: 0, width: width, height: 1.5)
                    button.titleLabel?.font = UIFont.systemFont(ofSize: self.config.blockFont + 1)
